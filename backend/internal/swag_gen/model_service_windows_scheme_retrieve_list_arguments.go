/*
 * Bumbal Client Api
 *
 * Bumbal API documentation
 *
 * API version: 2.0
 * Contact: info@bumbal.eu
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// ServiceWindowsSchemeRetrieveListArguments struct for ServiceWindowsSchemeRetrieveListArguments
type ServiceWindowsSchemeRetrieveListArguments struct {
	Options *ServiceWindowsSchemeOptionsModel `json:"options,omitempty"`
	Filters *ServiceWindowsSchemeFiltersModel `json:"filters,omitempty"`
	// 
	Limit *int64 `json:"limit,omitempty"`
	// 
	Offset *int64 `json:"offset,omitempty"`
	// 
	SearchText *string `json:"search_text,omitempty"`
	// 
	CountOnly *bool `json:"count_only,omitempty"`
	// 
	AsList *bool `json:"as_list,omitempty"`
	// Sorting Column
	SortingColumn *string `json:"sorting_column,omitempty"`
	// Sorting Direction
	SortingDirection *string `json:"sorting_direction,omitempty"`
}

// NewServiceWindowsSchemeRetrieveListArguments instantiates a new ServiceWindowsSchemeRetrieveListArguments object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewServiceWindowsSchemeRetrieveListArguments() *ServiceWindowsSchemeRetrieveListArguments {
	this := ServiceWindowsSchemeRetrieveListArguments{}
	return &this
}

// NewServiceWindowsSchemeRetrieveListArgumentsWithDefaults instantiates a new ServiceWindowsSchemeRetrieveListArguments object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewServiceWindowsSchemeRetrieveListArgumentsWithDefaults() *ServiceWindowsSchemeRetrieveListArguments {
	this := ServiceWindowsSchemeRetrieveListArguments{}
	return &this
}

// GetOptions returns the Options field value if set, zero value otherwise.
func (o *ServiceWindowsSchemeRetrieveListArguments) GetOptions() ServiceWindowsSchemeOptionsModel {
	if o == nil || o.Options == nil {
		var ret ServiceWindowsSchemeOptionsModel
		return ret
	}
	return *o.Options
}

// GetOptionsOk returns a tuple with the Options field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ServiceWindowsSchemeRetrieveListArguments) GetOptionsOk() (*ServiceWindowsSchemeOptionsModel, bool) {
	if o == nil || o.Options == nil {
		return nil, false
	}
	return o.Options, true
}

// HasOptions returns a boolean if a field has been set.
func (o *ServiceWindowsSchemeRetrieveListArguments) HasOptions() bool {
	if o != nil && o.Options != nil {
		return true
	}

	return false
}

// SetOptions gets a reference to the given ServiceWindowsSchemeOptionsModel and assigns it to the Options field.
func (o *ServiceWindowsSchemeRetrieveListArguments) SetOptions(v ServiceWindowsSchemeOptionsModel) {
	o.Options = &v
}

// GetFilters returns the Filters field value if set, zero value otherwise.
func (o *ServiceWindowsSchemeRetrieveListArguments) GetFilters() ServiceWindowsSchemeFiltersModel {
	if o == nil || o.Filters == nil {
		var ret ServiceWindowsSchemeFiltersModel
		return ret
	}
	return *o.Filters
}

// GetFiltersOk returns a tuple with the Filters field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ServiceWindowsSchemeRetrieveListArguments) GetFiltersOk() (*ServiceWindowsSchemeFiltersModel, bool) {
	if o == nil || o.Filters == nil {
		return nil, false
	}
	return o.Filters, true
}

// HasFilters returns a boolean if a field has been set.
func (o *ServiceWindowsSchemeRetrieveListArguments) HasFilters() bool {
	if o != nil && o.Filters != nil {
		return true
	}

	return false
}

// SetFilters gets a reference to the given ServiceWindowsSchemeFiltersModel and assigns it to the Filters field.
func (o *ServiceWindowsSchemeRetrieveListArguments) SetFilters(v ServiceWindowsSchemeFiltersModel) {
	o.Filters = &v
}

// GetLimit returns the Limit field value if set, zero value otherwise.
func (o *ServiceWindowsSchemeRetrieveListArguments) GetLimit() int64 {
	if o == nil || o.Limit == nil {
		var ret int64
		return ret
	}
	return *o.Limit
}

// GetLimitOk returns a tuple with the Limit field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ServiceWindowsSchemeRetrieveListArguments) GetLimitOk() (*int64, bool) {
	if o == nil || o.Limit == nil {
		return nil, false
	}
	return o.Limit, true
}

// HasLimit returns a boolean if a field has been set.
func (o *ServiceWindowsSchemeRetrieveListArguments) HasLimit() bool {
	if o != nil && o.Limit != nil {
		return true
	}

	return false
}

// SetLimit gets a reference to the given int64 and assigns it to the Limit field.
func (o *ServiceWindowsSchemeRetrieveListArguments) SetLimit(v int64) {
	o.Limit = &v
}

// GetOffset returns the Offset field value if set, zero value otherwise.
func (o *ServiceWindowsSchemeRetrieveListArguments) GetOffset() int64 {
	if o == nil || o.Offset == nil {
		var ret int64
		return ret
	}
	return *o.Offset
}

// GetOffsetOk returns a tuple with the Offset field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ServiceWindowsSchemeRetrieveListArguments) GetOffsetOk() (*int64, bool) {
	if o == nil || o.Offset == nil {
		return nil, false
	}
	return o.Offset, true
}

// HasOffset returns a boolean if a field has been set.
func (o *ServiceWindowsSchemeRetrieveListArguments) HasOffset() bool {
	if o != nil && o.Offset != nil {
		return true
	}

	return false
}

// SetOffset gets a reference to the given int64 and assigns it to the Offset field.
func (o *ServiceWindowsSchemeRetrieveListArguments) SetOffset(v int64) {
	o.Offset = &v
}

// GetSearchText returns the SearchText field value if set, zero value otherwise.
func (o *ServiceWindowsSchemeRetrieveListArguments) GetSearchText() string {
	if o == nil || o.SearchText == nil {
		var ret string
		return ret
	}
	return *o.SearchText
}

// GetSearchTextOk returns a tuple with the SearchText field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ServiceWindowsSchemeRetrieveListArguments) GetSearchTextOk() (*string, bool) {
	if o == nil || o.SearchText == nil {
		return nil, false
	}
	return o.SearchText, true
}

// HasSearchText returns a boolean if a field has been set.
func (o *ServiceWindowsSchemeRetrieveListArguments) HasSearchText() bool {
	if o != nil && o.SearchText != nil {
		return true
	}

	return false
}

// SetSearchText gets a reference to the given string and assigns it to the SearchText field.
func (o *ServiceWindowsSchemeRetrieveListArguments) SetSearchText(v string) {
	o.SearchText = &v
}

// GetCountOnly returns the CountOnly field value if set, zero value otherwise.
func (o *ServiceWindowsSchemeRetrieveListArguments) GetCountOnly() bool {
	if o == nil || o.CountOnly == nil {
		var ret bool
		return ret
	}
	return *o.CountOnly
}

// GetCountOnlyOk returns a tuple with the CountOnly field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ServiceWindowsSchemeRetrieveListArguments) GetCountOnlyOk() (*bool, bool) {
	if o == nil || o.CountOnly == nil {
		return nil, false
	}
	return o.CountOnly, true
}

// HasCountOnly returns a boolean if a field has been set.
func (o *ServiceWindowsSchemeRetrieveListArguments) HasCountOnly() bool {
	if o != nil && o.CountOnly != nil {
		return true
	}

	return false
}

// SetCountOnly gets a reference to the given bool and assigns it to the CountOnly field.
func (o *ServiceWindowsSchemeRetrieveListArguments) SetCountOnly(v bool) {
	o.CountOnly = &v
}

// GetAsList returns the AsList field value if set, zero value otherwise.
func (o *ServiceWindowsSchemeRetrieveListArguments) GetAsList() bool {
	if o == nil || o.AsList == nil {
		var ret bool
		return ret
	}
	return *o.AsList
}

// GetAsListOk returns a tuple with the AsList field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ServiceWindowsSchemeRetrieveListArguments) GetAsListOk() (*bool, bool) {
	if o == nil || o.AsList == nil {
		return nil, false
	}
	return o.AsList, true
}

// HasAsList returns a boolean if a field has been set.
func (o *ServiceWindowsSchemeRetrieveListArguments) HasAsList() bool {
	if o != nil && o.AsList != nil {
		return true
	}

	return false
}

// SetAsList gets a reference to the given bool and assigns it to the AsList field.
func (o *ServiceWindowsSchemeRetrieveListArguments) SetAsList(v bool) {
	o.AsList = &v
}

// GetSortingColumn returns the SortingColumn field value if set, zero value otherwise.
func (o *ServiceWindowsSchemeRetrieveListArguments) GetSortingColumn() string {
	if o == nil || o.SortingColumn == nil {
		var ret string
		return ret
	}
	return *o.SortingColumn
}

// GetSortingColumnOk returns a tuple with the SortingColumn field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ServiceWindowsSchemeRetrieveListArguments) GetSortingColumnOk() (*string, bool) {
	if o == nil || o.SortingColumn == nil {
		return nil, false
	}
	return o.SortingColumn, true
}

// HasSortingColumn returns a boolean if a field has been set.
func (o *ServiceWindowsSchemeRetrieveListArguments) HasSortingColumn() bool {
	if o != nil && o.SortingColumn != nil {
		return true
	}

	return false
}

// SetSortingColumn gets a reference to the given string and assigns it to the SortingColumn field.
func (o *ServiceWindowsSchemeRetrieveListArguments) SetSortingColumn(v string) {
	o.SortingColumn = &v
}

// GetSortingDirection returns the SortingDirection field value if set, zero value otherwise.
func (o *ServiceWindowsSchemeRetrieveListArguments) GetSortingDirection() string {
	if o == nil || o.SortingDirection == nil {
		var ret string
		return ret
	}
	return *o.SortingDirection
}

// GetSortingDirectionOk returns a tuple with the SortingDirection field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ServiceWindowsSchemeRetrieveListArguments) GetSortingDirectionOk() (*string, bool) {
	if o == nil || o.SortingDirection == nil {
		return nil, false
	}
	return o.SortingDirection, true
}

// HasSortingDirection returns a boolean if a field has been set.
func (o *ServiceWindowsSchemeRetrieveListArguments) HasSortingDirection() bool {
	if o != nil && o.SortingDirection != nil {
		return true
	}

	return false
}

// SetSortingDirection gets a reference to the given string and assigns it to the SortingDirection field.
func (o *ServiceWindowsSchemeRetrieveListArguments) SetSortingDirection(v string) {
	o.SortingDirection = &v
}

func (o ServiceWindowsSchemeRetrieveListArguments) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Options != nil {
		toSerialize["options"] = o.Options
	}
	if o.Filters != nil {
		toSerialize["filters"] = o.Filters
	}
	if o.Limit != nil {
		toSerialize["limit"] = o.Limit
	}
	if o.Offset != nil {
		toSerialize["offset"] = o.Offset
	}
	if o.SearchText != nil {
		toSerialize["search_text"] = o.SearchText
	}
	if o.CountOnly != nil {
		toSerialize["count_only"] = o.CountOnly
	}
	if o.AsList != nil {
		toSerialize["as_list"] = o.AsList
	}
	if o.SortingColumn != nil {
		toSerialize["sorting_column"] = o.SortingColumn
	}
	if o.SortingDirection != nil {
		toSerialize["sorting_direction"] = o.SortingDirection
	}
	return json.Marshal(toSerialize)
}

type NullableServiceWindowsSchemeRetrieveListArguments struct {
	value *ServiceWindowsSchemeRetrieveListArguments
	isSet bool
}

func (v NullableServiceWindowsSchemeRetrieveListArguments) Get() *ServiceWindowsSchemeRetrieveListArguments {
	return v.value
}

func (v *NullableServiceWindowsSchemeRetrieveListArguments) Set(val *ServiceWindowsSchemeRetrieveListArguments) {
	v.value = val
	v.isSet = true
}

func (v NullableServiceWindowsSchemeRetrieveListArguments) IsSet() bool {
	return v.isSet
}

func (v *NullableServiceWindowsSchemeRetrieveListArguments) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableServiceWindowsSchemeRetrieveListArguments(val *ServiceWindowsSchemeRetrieveListArguments) *NullableServiceWindowsSchemeRetrieveListArguments {
	return &NullableServiceWindowsSchemeRetrieveListArguments{value: val, isSet: true}
}

func (v NullableServiceWindowsSchemeRetrieveListArguments) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableServiceWindowsSchemeRetrieveListArguments) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


